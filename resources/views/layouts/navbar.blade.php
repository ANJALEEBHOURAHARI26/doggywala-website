...
<div id="content-wrapper" class="d-flex flex-column">
  ...
  <script> 
    ...
    const current_date_time = response.current_date_time;
    // console.log(count);
    // Suppose appointmentlist fetched from AJAX
 

let oldAppointmentCount = 0;

// console.log(oldAppointmentCount);

    let html = '';
    // appointmentlist.forEach(function(item) {
    appointmentlist.forEach((item, index) => {

        //  const originalTime = item.appointment_date; // "2025-04-28 10:59:00"
    ...
    if (item.appointment_date) {
        ...
        date.setMinutes(date.getMinutes() - 30); // 30 minute peeche
        formattedTime1 = formatDate(date);
    }
    console.log(formattedTime1);
        if(current_date_time == formattedTime1){
        ...
        // Removed appointment_date check for now
        const customer = item.customer?.name ?? 'Unknown';
        const notes = item.notes ?? '';
        const formattedTime = item.appointment_date ? new Date(item.appointment_date).toLocaleString() : '';
    ...
    setInterval(fetchAppointmentCount, 10000); 
...
function formatDate(date) {
    ...
    const month = String(date.getMonth() + 1).padStart(2, '0'); // month 0 se start hota hai JS me
